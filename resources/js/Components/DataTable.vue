...
function changeOrder(field, order){
    emit('sortingOrder', {orderField : field, orderBy : order == 'asc' ? 'desc' : 'asc'});
}
            ...
            <template v-for="col in displayedColumns" :key="col.ref">
                ...
                <th v-if="(['select','action']).indexOf(col.ref) == -1" scope="col" class="px-6 py-3">
                    ...
                    <template v-if="orderField == col.ref">
                        <span class="arrow active" :class="orderBy" @click="changeOrder(orderField, orderBy)"></span>
                    </template>
                    <template v-else>
                        <span class="arrow desc" @click="changeOrder(col.ref, 'desc')"></span>
                    </template>
                </th>
